...
public interface CustomerRepository extends JpaRepository<CustomerEntity, String> {
    @Query(value = "SELECT * FROM allowme_fake cs WHERE cs.username = ?1", nativeQuery = true)
    CustomerEntity findByUsername(String name);

    @Query(value = "SELECT id FROM allowme_fake cs WHERE cs.username = ?1", nativeQuery = true)
